...
typedef struct _DeviceTX_ISNF
{
	
	uint8 send_flag;
	uint8 UnRecive_Time;//未接收时间间隔
	uint8 Recive_Flag;//接收标志
	uint8 Flag_100ms;  //100ms  Flag
}DeviceTx_Inf;

typedef struct _DeviceError_ISNF
{
	uint8 E3_Find_Flag;
	uint8 E3_Find_Time;//故障的持续时间，100ms

	uint8 E9_Find_Flag;
	uint8 E9_Find_Time;//故障的持续时间，100ms
}DError_Inf;

typedef struct _sys_flags
{
	uint16 Work_TimeMins;  //运行时间分钟
	uint16 Last_Blow_EndTime;
...
extern DeviceTx_Inf JiaYao_Inf;
