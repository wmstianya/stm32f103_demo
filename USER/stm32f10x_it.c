		...
		if(time_100ms >= 100)
			{
				...
				if(ERR_Inf.E9_Find_Flag)
					{
						ERR_Inf.E9_Find_Time ++;
					}
				else
					{
				...
				if(ERR_Inf.E3_Find_Flag)
					{
						ERR_Inf.E3_Find_Time ++;
					}
				else
					{
					...
					if(sys_data.Data_10H == 2)
						{
							if(sys_flag.flame_state)
								{
									sys_flag.Work_TimeMins ++; //对工作时间计时
								}
						}
				}			
