...
void read_serial_data(void)
{
	...
	/**************************虚拟标志位    输入脚13**计量泵的在线标志**********************/ 	
	if(sys_flag.Address_Number == 0)
		{
			if(JiaYao_Inf.Recive_Flag)
    			sys_flag.Inputs_State = SET_BIT_N(sys_flag.Inputs_State,13);
			else
				sys_flag.Inputs_State = CLR_BIT_N(sys_flag.Inputs_State,13);
		}
